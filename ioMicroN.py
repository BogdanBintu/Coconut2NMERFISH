...
class decoder_simple():
    ...
    def get_XH(self,fov,set_,ncols=3,nbits=16,th_h=0,filter_tag = 'MER'):
        self.set_ = set_
        save_folder = self.save_folder
    ...
    def get_icodes(self,nmin_bits=4,method = 'top4',redo=False,norm_brightness=None,nbits=48,is_unique=True,save=False):    
        ...
        n_on_bits = nmin_bits
        lens = self.lens
        res_unfolder = self.res_unfolder
        Mlen = np.max(lens)
        ...
        scores_bits = np.zeros([len(lens),nbits])
        ...
        if method == 'top4':
            ...
            inds_spotsN =  np.zeros([len(lens),nbits],dtype=int)-1
...
class get_dapi_features:
    ...
    def get_X_plus_minus(self):
        #load dapi
        im1 = self.im
        self.Xh_plus = get_local_maxfast_tensor(im1,th_fit=3,delta=5,delta_fit=3)
        self.Xh_minus = get_local_maxfast_tensor(-im1,th_fit=3,delta=5,delta_fit=3)
...
def get_best_translation_points(X,X_ref,resc=5,pad=10,target=3,return_counts=False):
    ...
    if return_counts:
        return tzxy,Npts
    return tzxy
def plot_statisticsV2(dec):
    if hasattr(dec,'im_segm_'):
        ncells = len(np.unique(dec.im_segm_))-1
    else:
        ncells = 1
    icds,ncds = np.unique(dec.icodesN[dec.thresholded],return_counts=True)
    good_igns = [ign for ign,gn in enumerate(dec.gns_names) if 'blank' not in gn.lower()]
    kp = np.in1d(icds,good_igns)
    ncds = ncds/ncells
    plt.figure()
    ...
    plt.title(str(np.round(np.mean(ncds[~kp])/np.mean(ncds[kp]),3)))
    plt.legend()
def get_stats_D_H_cor(dec,icol=0,plt_val=False,dic_th={'cor':[0.6,0.6],'H':[9,9],'D':[1.25,1.25]}):
    ibad = [ign for ign,gn in enumerate(dec.gns_names) if 'blank' in gn.lower()]
    is_bad = np.in1d(dec.icodesN,ibad)
    icols = np.unique(dec.XH_pruned[:,0,-2].astype(int))
    XC = np.mean(dec.XH_pruned[:,:,:3],axis=1)[:,np.newaxis]
    Dist = dec.XH_pruned[:,:,:3]-XC
    X = np.max(np.linalg.norm(Dist,axis=-1),axis=1)

    XD = X.copy()
    XH = np.log(np.min(dec.XH_pruned[:,:,-3],axis=1))
    Xcor = np.min(dec.XH_pruned[:,:,-6],axis=1)
    XF = np.array([XD,XH,Xcor])
    Xth = np.zeros_like(XF)
    for icol in icols:
        is_col = dec.XH_pruned[:,0,-2]==icol
        Xth[0][is_col]=dic_th['D'][icol]
        Xth[1][is_col]=dic_th['H'][icol]
        Xth[2][is_col]=dic_th['cor'][icol]
    flips = np.array([-1,1,1])[:,np.newaxis]
    Xth = Xth*flips
    XF = XF*flips
    keep = np.all(XF>Xth,axis=0)
    for icol in icols: 
        if plt_val:
            plt.figure()
            plt.title('Max distance to center - col '+str(icol))
            plt.hist(XD[(~is_bad)&is_col],bins=200,density=True,label='genes');
            plt.hist(XD[is_bad&is_col],bins=200,density=True,alpha=0.5,label='blank');
            plt.legend()
    
            plt.figure()
            plt.title('Min brightness - col '+str(icol))
            plt.hist(XH[(~is_bad)&is_col],bins=200,density=True,label='genes');
            plt.hist(XH[is_bad&is_col],bins=200,density=True,alpha=0.5,label='blank');
            plt.legend()
    
    
            plt.figure()
            plt.title('Min corr - col '+str(icol))
            plt.hist(Xcor[(~is_bad)&is_col],bins=500,density=True);
            plt.hist(Xcor[is_bad&is_col],bins=500,density=True,alpha=0.5,label='blank');
            plt.legend()
    dec.thresholded = keep
    return keep